/*$(function() {
    ...
    if ($('main.order').length) {
        ...
        for (let item of basket) {
            let hlpstr = '<tr data-id="'+item.id+'"><th scope="row" class="index"></th><td class="name">'+item.name+'</td><td class="qty"><span class="minus">&#xe90c;</span><strong>'+item.quantity+'</strong><span class="plus">&#xe90b;</span></td><td class="price"><span>'+item.price+'</span></td><td class="sum"><span></span></td><td class="delete">&#xe90a;</td></tr>';
            point.append(hlpstr);
            count++;
        }
        orderReCount();
        ...
        $('main.order form .submit').click(function(){
            ...
            if (!form.mail.value) {
                $('form #mail').addClass('is-invalid').parents('.mb-3').append('<div class="invalid-feedback">Должен быть указан E-mail!</div>');
                valid = false;
            }
            ...
            /*if (!form.agree.cheked) {
                $('form #agree').is(':checked').addClass('is-invalid').parents('.mb-3').append('<div class="invalid-feedback">Необходимо Ваше согласие на обработку персональных данных</div>');
                valid = false;    <<<<<<<<<<<<<<< Я втираю себе какую-то дичь
            }*/
            ...
            if($("#agree").is(":checked")) {
       /////////////////////////////////////// сюда ничего не нужно?
            }
            else {
                $('form #agree').addClass('is-invalid').parents('.mb-3').append('<div class="invalid-feedback">Необходимо Ваше согласие на обработку персональных данных</div>');
                valid = false;
        }

            if($("#сaptcha").val() == "ФЛЮГЕГЕХАЙМЕН" || $("#сaptcha").val() =="флюгегехаймен") {
            }
            else {
                $('form #сaptcha').addClass('is-invalid').parents('.mb-3').append('<div class="invalid-feedback">Введенное слово не верное</div>');
                valid = false;
            } 
            
        /*if (!form.сaptcha.value) {
            $('form #сaptcha').addClass('is-invalid').parents('.mb-3').append('<div class="invalid-feedback">Должен быть указан E-mail!</div>');
            valid = false;
        } */ 
            ...
            if (valid) {
                ...
                fetch('https://jsonplaceholder.typicode.com/posts', { // отправляем заказ
                    ...
                    $('.modal').append('<p>Ваш заказ оформлен под номером ' + json.id + '. Ожидайте звонка от оператора.</p>'); // выводим номер заказа клиенту
